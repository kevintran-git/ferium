...
[![licence badge](https://img.shields.io/github/license/kevintran-git/ferium)](https://github.com/kevintran-git/ferium/blob/main/LICENSE.txt)
[![build.yml](https://github.com/kevintran-git/ferium/actions/workflows/build.yml/badge.svg)](https://github.com/kevintran-git/ferium/actions/workflows/build.yml)

Hopper is a fork of [ferium](https://github.com/gorilla-devs/ferium) with additional bug fixes and features layered on top — see `UPGRADE_ROADMAP.md` for what changed and why. Under the hood it still uses [ferinth](https://github.com/gorilla-devs/ferinth) and [furse](https://github.com/gorilla-devs/furse), Rust wrappers for the official Modrinth and CurseForge APIs respectively.

Hopper is a fast and feature rich CLI program for downloading and updating Minecraft mods from [Modrinth](https://modrinth.com/mods), [CurseForge](https://curseforge.com/minecraft/mc-mods), and [GitHub Releases](https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases), and modpacks from [Modrinth](https://modrinth.com/modpacks) and [CurseForge](https://curseforge.com/minecraft/modpacks).
...
- Upgrade all your mods to the latest compatible version in one command, `hopper upgrade`
  - Hopper checks that the version being downloaded is the latest one that is compatible with the configured mod loader and Minecraft version
- Download and install the latest version of your modpack in one command, `hopper modpack upgrade`
- Create multiple profiles and configure different mod loaders, Minecraft versions, output directories, and mods for each

## Installation

Hopper executables from GitHub Releases do not require any external dependencies at runtime.  
...
[![GitHub Releases](https://img.shields.io/github/v/release/kevintran-git/ferium?color=bright-green&label=github%20releases)](https://github.com/kevintran-git/ferium/releases)
> [!IMPORTANT]
> You will have to manually download and install every time there is a new update.

1. Download the asset suitable for your operating system from the [latest release](https://github.com/kevintran-git/ferium/releases/latest)
2. Unzip the file and move the `hopper` executable to a folder in your `PATH`, e.g. `~/bin`
3. Remember to check the releases page for any updates!

> [!NOTE]
> If real upstream ferium is already installed, remove it first — only one binary can own the `ferium`/`hopper` command on your `PATH` at a time. Your existing ferium config (profiles, tracked mods) is picked up automatically either way; see [Program Configuration](#program-configuration).
...
```bash
cargo install --git https://github.com/kevintran-git/ferium
```

This installs the `hopper` binary to your Cargo binary directory (`~/.cargo/bin` by default).

## Overview / Help Page

> [!NOTE]
> A lot of Hopper's backend is implemented in [libium](https://github.com/kevintran-git/ferium/tree/main/libium).  
> It deals with things such as the config, adding mod(pack)s, upgrading, file pickers, etc.

### Program Configuration

Hopper stores profile and modpack information in its config file. By default, this is located at
...
The on-disk folder is still named `ferium`, not `hopper` — this is intentional, so an existing real-ferium config is picked up automatically with no migration step.

You can change this in 2 ways; by setting the `HOPPER_CONFIG_FILE` environment variable,
...
- [Create a new profile](#creating) by running `hopper profile create` and entering the details for your profile.
...
The directory defaults to your profile's output directory. Some mods are available on both Modrinth and CurseForge; hopper will prefer Modrinth by default, but you can choose CurseForge instead using the `--platform` flag.
...
So to add [Sodium](https://modrinth.com/mod/sodium), you can run `hopper add sodium` or `hopper add AANobbMI`.
...
So to add [Terralith](https://www.curseforge.com/minecraft/mc-mods/terralith), you should run `hopper add 513688`.

#### GitHub
```bash
hopper add owner/name
```
...
So to add [Sodium](https://github.com/CaffeineMC/sodium), you should run `hopper add CaffeineMC/sodium`.

> [!IMPORTANT]
> The GitHub repository needs to upload JAR files to their _Releases_ for hopper to download, or else it will refuse to be added.

#### User Mods

If you want to use files that are not downloadable by hopper, place them in a subfolder called `user` in the output directory. Files here will be copied to the output directory when upgrading.
...
So to add [Fabulously Optimized](https://modrinth.com/modpack/fabulously-optimized), you can run `hopper modpack add fabulously-optimized` or `hopper modpack add 1KVo5zza`.
...
> [!WARNING]
> If your output directory is not empty when setting it, hopper will offer to create a backup.  
...
> [!TIP]
> When upgrading, any files not downloaded by hopper will be moved to the `.old` folder in the output directory.  
> See [user mods](#user-mods) for information on how to add mods that hopper cannot download.
...
You can list out all the mods in your current profile by running `hopper list`. If you want to see more information about them, you can use `hopper list -v` or `hopper list --verbose`.

You can remove any of your mods using `hopper remove`; just select the ones you would like to remove using the space key, and press enter once you're done. You can also provide the names, IDs, or slugs of the mods as arguments.

> [!TIP]
> Older versions did not have the ability to remove mods by their slug, since it wasn't stored in the config. If you are upgrading from an older version, you will initially not be able to remove mods by their slugs. You can run `hopper list -v` to load the slugs into the profile.

> [!IMPORTANT]
> Both mod names and GitHub repository identifiers are case insensitive.  
> Mod names with spaces have to be given in quotes (`hopper remove "ok zoomer"`) or the spaces should be escaped (usually `hopper remove ok\ zoomer`, but depends on the shell).

#### Check Overrides

If some mod is supposed to be compatible with your game version and mod loader, but hopper does not download it, [create an issue](https://github.com/kevintran-git/ferium/issues/new?labels=bug&template=bug-report.md) if you think it's a bug.
...
For example, [Just Enough Items](https://www.curseforge.com/minecraft/mc-mods/jei) does not specify the mod loader for older Minecraft versions such as `1.12.2`. In this case, you would add JEI by running `hopper add 238222 --ignore-mod-loader` so that the mod loader check is disabled.  
...
You can create a profile by running `hopper profile create` and specifying the following:
...
> [!NOTE]
> Hopper will automatically switch to the newly created profile
...
You can configure these same settings afterwards by running `hopper profile configure`. Again, you can provide these settings as flags.

#### Manage

You can get information about the current profile by running `hopper profile` or `hopper profile info`, and about all the profiles you have by running `hopper profiles` or `hopper profile list`.  
Switch to a different profile using `hopper profile switch`.  
Delete a profile using `hopper profile delete` and selecting the profile you want to delete.

## Feature Requests

If you would like to make a feature request, check the [issue tracker](https://github.com/kevintran-git/ferium/issues?q=is%3Aissue+label%3Aenhancement) to see if the feature has already been added or is planned.
If not, [create a new issue](https://github.com/kevintran-git/ferium/issues/new/choose).
...
If you still have doubts, feel free to [create a discussion](https://github.com/kevintran-git/ferium/discussions/new?category=q-a) and I will try help you out.
